...
async function saveCommandes(action, payload = {}) {
  if (!APPS_SCRIPT_URL) return; // pas encore configuré
  try {
    await fetch(APPS_SCRIPT_URL, {
      method: 'POST',
      mode: 'no-cors', // Apps Script ne renvoie pas les headers CORS sur redirect
      headers: { 'Content-Type': 'text/plain' },
      body: JSON.stringify({ action, ...payload }),
    });
  } catch(e) { console.warn('Sync Google Sheet impossible:', e); }
}
...
function printNewBC() {
  ...
  <div class="sub">Boutique Contrées Vertes &nbsp;|&nbsp; <em>Non assujetti à la TVA</em></div>
...
function printReleve(supplier, dateDebut, dateFin) {
  ...
  <div class="sub">Boutique Contrées Vertes<br>
  ...
  Boutique Contrées Vertes - Généré le ${today}</div>
  <script>window.onload=()=>window.print()<\/script></body></html>`);
  win.document.close();
}
