...
function renderCmdArticlesTable(prods) {
  ...
  document.getElementById('cmd-articles-tbody').innerHTML = prods.map(p => {
    ...
    const total = qty > 0 && p.products_price > 0 ? fmtEur(qty * p.products_price) : '-';
...
function setCmdQty(id, val) {
  ...
  if (totalCell) totalCell.textContent = qty > 0 && p && p.products_price > 0 ? fmtEur(qty * p.products_price) : '-';
  const inp = document.getElementById('cmd-qty-' + id);
...
function updateRecap() {
  ...
  // Pour une commande fournisseur : total basé sur le prix d'achat HT
  ...
  // Non assujetti TVA : TTC = HT pour les achats
  document.getElementById('cmd-recap-total').innerHTML = `<span style="color:var(--txt2)">PA HT : ${fmtEur(totalAchatHT)}</span>&nbsp;&nbsp;<strong>Total commande : ${fmtEur(totalAchatHT)}</strong>`;
  if (lines.length === 0) {
...
function printNewBC() {
  ...
  const rows = articles.map(({p,qty}) => `<tr>
    ...
    <td style="text-align:right">${p.products_price>0?fmtEur(p.products_price):'-'}</td>
    <td style="text-align:right;font-weight:bold">${p.products_price>0?fmtEur(p.products_price*qty):'-'}</td>
  </tr>`).join('');
  ...
  <table><thead><tr><th>Réf.</th><th>Désignation</th><th class="text-right">Qté</th><th class="text-right">PA HT</th><th class="text-right">Total HT</th></tr></thead>
  <tbody>${rows}<tr class="total-row"><td colspan="4">TOTAL HT</td><td class="text-right">${fmtEur(totalHT)}</td></tr></tbody></table>
